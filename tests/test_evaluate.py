...
class Test_gf_simple:
	def test_generate_ETPs(self):
		...
		result = smut.make_result_dict_from_mutype_tree_alt(gf, mutype_tree, theta, rate_dict, ordered_mutype_list, max_k)
	...
	def test_probK(self):
		...
		mucount_total = 2
		mucount_fact_prod = 1 
		probk_deriv = smut.generate_equation(gf, (0,0), (1,1), (2,2), ordered_mutype_list, marginals)
...
class Test_zero_division:
	...
	def test_zero_div(self, get_gf):
		...
		gfEvaluatorObj = leval.gfEvaluator(gf, max_k, mutype_labels, exclude=[(2,3),])		
