...
class Test_taylor:
	def test_taylor_polynomial(self):
		...
		result = gfdiff.taylor_coeff_inverse_polynomial(denom, var_array[-1], diff_array, num_mutypes, dot_product, (4,4))
	...
	def test_taylor_polynomial_marginals(self):
		...
		num_mutypes = 2
		dot_product = gfdiff.simple_dot_product_setback(denom, var_array, 2)
		result = gfdiff.taylor_coeff_inverse_polynomial(denom, var_array[-1], diff_array, num_mutypes, dot_product, (3,3))
	...
	def test_taylor_exponential(self):
		...
		time = 2
		dot_product = gfdiff.simple_dot_product_setback(denom, var_array, 2)
		result = gfdiff.taylor_coeff_exponential(-time, denom, dot_product, diff_array, num_mutypes, var_array[-1], (4,4))
	...
	def test_prod_of_polynomials(self, subsetdict):
		...
		result = gfdiff.compile_non_inverted_eq(eq_matrix, size, mutype_array, mutype_shape_with_marg)(var_array)
	...
	def test_diff_inverse_laplace(self, eq_matrix, delta_in_nom, subsetdict):
		...
		result = gfdiff.compile_inverted_eq(eq_matrix_no_delta, size, subsetdict, delta_in_nom, mutype_array, mutype_shape_with_marg)(var_array, time).reshape(mutype_shape)
		print(result)
		#from symbolic eq
		subs_dict = {b:var_array[-1] for b in symbolic_var_array[-2:]}
	...
	def test_diff_inverse_laplace2(self, eq_matrix, delta_in_denom, delta_in_nom, subsetdict):
		...
		#	subsetdict[idx] = entry
		result_inverted_part = gfdiff.compile_inverted_eq(eq_matrix_no_delta[delta_in_denom], size, subsetdict, delta_in_nom, mutype_array, mutype_shape_with_marg)(var_array, time)
		result_non_inverted_part = gfdiff.compile_non_inverted_eq(eq_matrix_no_delta[~delta_in_denom], size, mutype_array, mutype_shape_with_marg)(var_array)
...
class Test_taylor_single_pop:
	...
	def test_combining_probabilities(self, size):
		...
		result_with_marginals = evaluate_no_marginal(gfEvalObj, theta, variable_array, time)
...
class Test_taylorIM:
	...
	def get_BT_object(self):
		...
		return gfmuts.BranchTypeCounter(sample_list, branchtype_dict=branchtype_dict_mat)

	@pytest.fixture(
	scope='class', 
	params=[
		([(1,2,0)], True, None, None,[72/125, np.array([1.0, 15/13, 5/2], dtype=np.float64), 10/3], 'DIV'),
		(None, None, [(2,1)], True, [312/625, np.array([0.0, 1.0, 13/6, 134369693800271/73829502088061], dtype=np.float64), 0.0],'MIG_BA'),
		([(1,2,0)], True, [(1,2)], True, [72/125, np.array([1.0, 15/13, 5/2, 21/10], dtype=np.float64), 10/3], 'IM_AB'),
		],
	ids=[
		'DIV',
		'MIG', 
...
class Test_epsilon:
	...
	scope='class', 
	params=[
		([(1,2,0)], True, [(2,1)], True, [0.5, np.array([1.0, .2, .3, .4], dtype=np.float64), 1.5], 'IM_BA_zerodivision'),
		],
	)
	def get_IM_gfobject(self, request, get_BT_object):
		return get_IM_gfobject_BT(request.param, get_BT_object)
...
"""legacy
#generate code to evaluate sage array
