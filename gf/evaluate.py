...
class gfEvaluatorLegacy:
	def __init__(self, gfobj, k_max, mutype_array):
		...
		self.subsetdict = gfdiff.product_subsetdict_marg(tuple(self.final_result_shape), mutype_array)
		...
		self.multiplier_matrix = gfdiff.taylor_to_probability_coeffs(mutype_array, self.final_result_shape, include_marginals=True)
	...
	def evaluate(self, theta, var, time):
		try:
			...
			results = self.evaluator(var, time)
		final_result_flat = gfdiff.iterate_eq_graph(self.dependency_sequence, self.eq_graph_array, results, self.subsetdict)
...
class gfEvaluator:
	def __init__(self, gfObj, MutationTypeCounter):
		...
		self.multiplier_matrix = gfdiff.taylor_to_probability_coeffs(MutationTypeCounter.all_mutypes, self.final_result_shape, include_marginals=True)
	...
	def make_product_subsetdict(self, MutationTypeCounter):
		product_subsetdict_with_gaps = gfdiff.product_subsetdict_marg(self.final_result_shape, MutationTypeCounter.all_mutypes)
		if self.simple_reshape:
